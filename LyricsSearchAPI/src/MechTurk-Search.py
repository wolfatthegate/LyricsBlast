...
def printResult(result, songtitle):
    print('')
...
def searchLyrics(doc):
    ...
    for eachtitle in mytitle: 
        ...
        if round(result[2],2) > 0.85: 
            ...
            titleMatched = True
            suggestions = suggestions + '\n' + printResult(result, eachtitle['title'])   
            updatequery = {'_id': doc['_id']}
            newvalue = { '$set': {'score': round(result[2],2), \
                            'suggestions': suggestions, \
                                'song': eachtitle['title'], 'found': "1"}}
            testTbl.update_one(updatequery, newvalue)
            break
    ...
    print(str(mylyrics.count()) + ' possible lyrics found.')
    with open('archived_tweets/log.txt', 'a') as file:
        ...
        for eachline in eachlyrics['lyrics2'].splitlines():
            ...
            if continue_test == True or stepBack == True or followUp == True:
                ...
                if result[2] > mid_score and result[3] > 3: 
                    suggestions = suggestions + '\n' + printResult(result, eachlyrics['title'])
    ...
    updatequery = {'_id': doc['_id']}
    newvalue = { '$set': {'score': round(score,2) , 'suggestions': suggestions, 'song': song, 'found': found}}
    testTbl.update_one(updatequery, newvalue)
